""" Minor tweak of the regular softmax cross entropy
that allows for weighted sum of instances in the batch.

Changed from
http://docs.chainer.org/en/stable/_modules/chainer/functions/loss/softmax_cross_entropy.html#softmax_cross_entropy

Note that this still averages the weighted sum after by the sum of the weights
after, so no need to normalize the weights.
"""
